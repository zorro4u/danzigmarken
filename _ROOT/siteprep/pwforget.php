...
require_once __DIR__.'/../sitemsg/pwforget.php';
...
class PWforgetPrep
{
    protected const MSG = PWforgetMsg::MSG;
    ...
    protected static function dataPreparation(): void
    {
        ...
        if (isset($_GET['send']) && strtoupper($_SERVER["REQUEST_METHOD"] === "POST")):
            ...
            if (empty($error_msg)):
                ...
                // Email in DB suchen
                $usr_data = PWforgetData::searchEmail($input_email);
                ...
                // okay... temporären Passwort-Code in DB schreiben, 2 Tage gültig
                    ...
                    $mailto1  = $input_email;
                    $subject1 = self::MSG[117];
                    $mailcontent1 = self::MSG[118].$name.",\n\n".
                        self::MSG[119]."\n".
                        self::MSG[120].' '.$pwcode_endtime_str.") ".
                        self::MSG[121].":\n\n".$pwcode_url."\n\n".
                        self::MSG[122]."\n\n".
                        self::MSG[123]."\n".
                        self::MSG[124]."\n";
                    ...
                    if ($email_send1) {
                        $success_msg =
                            self::MSG[113].' '.$usr_data['email'].' '.self::MSG[114].' <b>'.$pwcode_endtime_str.'</b>) '.self::MSG[115];
                        $show_form = False;
                    }
                    else {
                        $error_arr []= self::MSG[116];
                    };

                };  // temporären Passwort-Code in DB schreiben
            endif;  // Eingabeformular okay
