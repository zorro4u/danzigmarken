<?php
  ...
  */
 /*
 Plugin Name: IOSEC Anti Flood Security Gateway Module
 Plugin URI: https://sourceforge.net/projects/iosec/
 ...
 */
 ...
 ///////////////////////////////////////////////////////////////////////////////////////////
     ...
     */
     ...
     if (!is_dir(WP_CONTENT_DIR . '/iosec_admin/')) {
         mkdir(WP_CONTENT_DIR . '/iosec_admin/', 0, true);
     }
     if (substr(sprintf('%o', fileperms(WP_CONTENT_DIR . '/iosec_admin/')), -4) < 755) {
         chmod(WP_CONTENT_DIR . '/iosec_admin/', 0755);
     }
     
     //Create .htaccess file...
     
     if (!file_exists(WP_CONTENT_DIR . '/iosec_admin/.htaccess')) {
         $icerik = "deny from all";
         
         $handle = fopen(WP_CONTENT_DIR . '/iosec_admin/.htaccess', 'w+');
     ...
     $banlisttemp = WP_CONTENT_DIR . '/iosec_admin/banlisttemp';
     $whitelist   = WP_CONTENT_DIR . '/iosec_admin/whitelist';
	 $excluded    = WP_CONTENT_DIR . '/iosec_admin/excluded';
     $ips         = WP_CONTENT_DIR . '/iosec_admin/ips';
     $banlist     = WP_CONTENT_DIR . '/iosec_admin/banlist';
	...
	<noscript>
     ...
     if (!is_writable($banlisttemp)) {
         fopen($banlisttemp, 'a');
         
     }
     if (!is_writable($banlist)) {
         fopen($banlist, 'a');
...
<tr>
                ...
                <div align="center">
					...
					<img border="0" src=<?php
                     echo (plugin_dir_url(__FILE__) . "iosec_content/logo.png");
?> ></p>
...
<?php
                     ...
                     if (($error <> null) || ($hatali == 1)) {
                         if ($debug_info == "y") {
                             ...
                             echo "Microsecond: " . (time() + microtime());
                         }
